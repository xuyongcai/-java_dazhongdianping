...
public class OrdersServiceImpl implements OrdersService {
	...
	public List<OrdersDto> searchByPage(OrdersDto ordersDto){
		...
		List<Orders> ordersList = ordersDao.select(condition);
		for(Orders orders : ordersList){
			OrdersDto OrdersDtoTemp = new OrdersDto();
			result.add(OrdersDtoTemp);
			BeanUtils.copyProperties(orders, OrdersDtoTemp);
			OrdersDtoTemp.setImg(businessImageUrl + orders.getBusiness().getImgFileName());
			OrdersDtoTemp.setCount(orders.getBusiness().getNumber());
